...
public class UserMgrController extends BaseController {
     ...
     */
    @Permission
    @RequestMapping("/user_complete/{userId}")
    public String userComplete(@PathVariable Integer userId, Model model) {
        if (ToolUtil.isEmpty(userId)) {
            throw new BussinessException(BizExceptionEnum.REQUEST_NULL);
        }
        assertAuth(userId);
        ...
        model.addAttribute("deptName", ConstantFactory.me().getDeptName(user.getDeptid()));
        LogObjectHolder.me().set(user);
        return PREFIX + "user_complete.html";
    }
     ...
     */
    @Permission
    @RequestMapping("/toTranslatePage/{userId}")
    public String toTranslatePage(@PathVariable Integer userId, Model model) {
        if (ToolUtil.isEmpty(userId)) {
            throw new BussinessException(BizExceptionEnum.REQUEST_NULL);
        }
        assertAuth(userId);
        ...
        model.addAttribute("deptName", ConstantFactory.me().getDeptName(user.getDeptid()));
        LogObjectHolder.me().set(user);
        return PREFIX + "user_translate.html";
    }
     ...
     */
    @Permission
    @RequestMapping("/toLeavePage/{userId}")
    public String toLeavePage(@PathVariable Integer userId, Model model) {
        if (ToolUtil.isEmpty(userId)) {
            throw new BussinessException(BizExceptionEnum.REQUEST_NULL);
        }
        assertAuth(userId);
    ...
    public Object list(@RequestParam(required = false) String name, @RequestParam(required = false) String beginTime, @RequestParam(required = false) String endTime
    		,@RequestParam(required = false) Integer onOrLeave, @RequestParam(required = false) Integer deptid) {
        if (ShiroKit.isAdmin()) {
            List<Map<String, Object>> users = managerDao.selectUsers(null, name, beginTime, endTime, deptid,onOrLeave);
            return new UserWarpper(users).warp();
        } else {
            DataScope dataScope = new DataScope(ShiroKit.getDeptDataScope());
            List<Map<String, Object>> users = managerDao.selectUsers(dataScope, name, beginTime, endTime, deptid,onOrLeave);
            return new UserWarpper(users).warp();
        }
    }
    ...
    @ResponseBody
    public Tip translate(@RequestParam Integer userid,@RequestParam String date,@RequestParam Integer usertype) {
        if (ToolUtil.isEmpty(userid)) {
            throw new BussinessException(BizExceptionEnum.REQUEST_NULL);
        }
        //不能冻结超级管理员
      ...
      //  assertAuth(userId);
        if(usertype==4) {
        	//办理离职
        	this.managerDao.translate(userid, usertype,date);
     ...
     * @throws NoPermissionException
     */
    @RequestMapping("/mgr/completeInfo")
    @Permission
    @ResponseBody
    public Tip completeInfo(@Valid UserTotalInfo user, BindingResult result) throws NoPermissionException {
        if (result.hasErrors()) {
            throw new BussinessException(BizExceptionEnum.REQUEST_NULL);
        }
        if (ShiroKit.hasRole(Const.ADMIN_NAME)) {
            this.userMapper.updateById(UserFactory.createTotalUser(user));
            return SUCCESS_TIP;
        } else {
            assertAuth(user.getId());
            ShiroUser shiroUser = ShiroKit.getUser();
     ...
     * @return
     */
    @RequestMapping(method = RequestMethod.POST, path = "/selectList")
    @ResponseBody
    public Object selectList() {
	    List<Map<String, Object>> users = managerDao.selectUsers(null, null, null, null, null,1);
	    return new UserWarpper(users).warp();
    }
     ...
     * @return
     */
    @RequestMapping(method = RequestMethod.POST, path = "/selectRankList")
    @ResponseBody
    public Object selectRankList() {

...
UserInfoDlg.collectData = function () {
    this.set('id').set('idcard').set('sex').set('usertype').set('avatar')
        .set('email').set('name').set('birthday').set('nationality').set('deptid').set('phone')
        .set('startWorkTime').set('jobtype').set('marriageStatus').set('politicalStatus').set('levelid')
        .set('email').set('direct').set('job').set('direct').set('entrydate');
};
...
                    for(var i = 0; i < data.length; i++){
                        var d = data[i];
                        oSelect.append('<option value="'+(d.id)+'">'+(d.levelName)+'</option>');
                    }
                }
	    });
...
$(function () {
    ...
    //初始化婚姻状况
    $("#marriageStatus").val($("#marriageStatusValue").val());
    //初始化职级
    $("#levelid").val($("#mlevelidValue").val());
